...
class CustomPlayer(DataPlayer):
    ...
    **********************************************************************
    ...
    def uct_search(self,
        ...
        print('tree policy')
        edge_node = self.tree_policy(root_n)
    ...
    def expand(self, node):
        ...
        new_node = next(item for item in self.context if item == new_node)
    ...
    def get_action(self, state):
        ...
        **********************************************************************
        ...
        if self.context is None:
            self.context = {CustomPlayer.Node(root_s)}
        iter = 0
        while True:
            print('search {}'.format(iter))
            action = self.uct_search(root_s)
            print('done {}'.format(iter))
            iter += 1
...
"""
    def default_policy(self, state):
        ...
        while not temp_state.terminal_test():
            max(state.actions(),
                key=lambda x: self.my_moves(state.result(x)) - self.opponent_moves(state.result(x)))
            a = random.choice(temp_state.actions())
            temp_state = temp_state.result(a)
